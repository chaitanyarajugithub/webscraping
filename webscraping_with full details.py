...
def print_hi(name):
    ...
    for a in range(1, len(country.options)):
                ...
                if (len(all_hey_elements) > 0):
                    for x in range(0, len(all_hey_elements)):
                        ...
                        if (urlstate is not True):
                            ...
                            contactpersononedetails.append("Not the same Domine")
                            contactpersontwodetails.append("Not the same Domine")
                            ...
                            if (canlen == 2):
                                ...
                                for a in soup.find_all('div', attrs={'class': 'contactperson_info'}, limit=1):
                                    ...
                                    print(name.strip())
                                    contactpersononedetails.append(name.strip())
                                    contactpersontwodetails.append("No details found")
                                    print(getdata(a, 'Name:'))
        ...
        df = pd.DataFrame({'Company Name': companyname, 'Company Link': companylink, 'City': cityop, 'Address': address, 'Phone': phone,
                           ...
                           'Contact Person2 Title': contactpersontwotitle, 'Contact person2 Email': contactpersontwoemail, 'Contact Person2 Mobile': contactpersontwomobile, 'Contact Person2 DirectLine': contactpersontwodirectline})
